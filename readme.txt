...
- WP 6.7+ admin: no more "text domain loaded too early" notices on the Social settings screen.

ENHANCEMENTS

- Social admin uses anonymous functions for `init`, sanitize, and submenu render.
...
- Minor CSS fix for Social Share Buttons.
...
- Put back the submission buttons into the forms.
...
- All Google shortcodes get the "pwp-" prefix. All shortcodes must be changed!

FIXES

- Fixed Google Calendar embed URL.
...
- Social buttons fixed to return the content.
...
- Fixed a condition for Social buttons to return the $content instead of nothing.

ENHANCEMENTS

- Code optimizations.
- Google Analytics code is loading only on frontend from now.
- Google Tag Manager's noscript code is hooked to wp_body_open only.
...
- New shortcode to display current year: `[pwp-ev]`
...
- Removed theme specific conditions from Google Tag Manager codes and implemented the new wp_body_open method.
...
- Added new shortcode: tel
- Added new shortcode: pwp-oldal
- Added new shortcode: pwp-bejegyzes
- Added new shortcode: pwp-fb-video
